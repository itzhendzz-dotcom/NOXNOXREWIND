...
MYMODCFG(net.noxxa.rewind, NOXXA REWIND, 0.1.1, henn)
...
ON_MOD_LOAD()
{
...
#ifndef AML32
    logger->Error("v0.1.1 targets GTA SA v2.00 / armeabi-v7a first.");
    aml->ShowToast(true, "NOXXA REWIND v0.1.1: 32-bit GTA SA v2.00 build required");
    return;
#endif
    ...
    {
        noxxa::RewindInput input = g_ui.PollInput();
    ...
    aml->ShowToast(false, "NOXXA REWIND v0.1.1 loaded - hold << to rewind");
}
