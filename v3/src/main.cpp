...
MYMODCFG(net.noxxa.rewind, NOXXA REWIND, 3.1.0, henn)
...
ON_MOD_LOAD() {
    logger->SetTag("NOXXA REWIND v3.1 CAUSAL");
#ifndef AML32
    logger->Error("v3.1 currently targets GTA SA v2.00 / armeabi-v7a.");
    aml->ShowToast(true, "NOXXA REWIND v3.1: 32-bit GTA SA v2.00 required");
    return;
#endif
    ...
    // Causal mode: zero by default. Historical playback uses real time, while
    // GTA AI/physics sees a frozen simulation clock.
    s.rewindTimeScale = cfg->GetFloat("TimeScale", 0.0f, "World");
    s.restoreWorldHealth = cfg->GetBool("RestoreHealth", true, "World");
    ...
    g_ui.Init(bx, by, bs);
    g_fx.Init(fx);
    g_core.Init(s, audioReady ? &g_audio : nullptr);

    Events::gameProcessEvent.before += []() { g_core.BeforeGameProcess(); };
    Events::gameProcessEvent.after += []() {
        const noxxa::RewindInput in = g_ui.PollInput();
        g_core.Tick(in.held, in.released, in.doubleTapped);
    };
    ...
    logger->Info("V3.1 causal loaded: %.2fs, %dHz, radius %.1f, max %d, simScale %.3f, health=%d, audio %.3fs",
                 s.maxRewindSeconds,
                 s.snapshotHz,
                 s.radius,
                 s.maxEntities,
                 s.rewindTimeScale,
                 s.restoreWorldHealth ? 1 : 0,
                 audioReady ? g_audio.DurationSeconds() : 0.0f);
    aml->ShowToast(false, "NOXXA REWIND v3.1 - causal rewind");
}
